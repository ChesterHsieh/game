...
8. **最終插畫記憶條件**
   - 必要配方清單由 `res://assets/data/epilogue-requirements.json` 單一來源維護。
...
- **MUT 讀取資料**
  ...
  - `recipes.json`（`R_authored` 與 recipe id 驗證）
- **MUT 提供查詢給其他系統**
...
- `epilogue-requirements.json` 空或缺失：記 error，阻止 epilogue 相關訊號誤觸發。
...
| 旋鈕 | 所在處 | 預設 | 安全範圍 | 影響 |
|---|---|---|---|---|
| `milestone_pct` | `mut-config.json` | `[0.15, 0.50, 0.80]` | (0,1]、嚴格遞增且不重複 | 靜默敘事節點觸發 |
| `partial_threshold` | `mut-config.json` | `1.0` | [0,1] | 最終記憶門檻鬆緊 |
| `epilogue-requirements.json` | 資料檔 | 依內容 | 長度 >= 1，id 必須有效 | 最終記憶必要配方 |
| `carry_forward` | 場景檔 | 首場常為 `[]` | 每場通常 0~10 筆 | 跨場景種子卡延續 |
| `force_unlock_all`（僅開發） | `debug-config.json` | `false` | 只允許 dev | 開發快速跳關 |

注意事項：
- `milestone_pct` 太密集會讓靜默狀態變更變成噪音。
- `partial_threshold < 0.5` 容易讓結局「得來太容易」。
- `epilogue-requirements` 包含不存在 id 會造成永遠無法完成，必須修檔，不可靜默移除。
- `debug-config.json` 必須由 export 設定排除於 release。
