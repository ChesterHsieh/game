...
> from this spec via `/create-scene save-for-italy`.
>
> **How to use this template**: fill every section top-to-bottom. Run `/scene-audit
> save-for-italy` after filling to catch missing references before committing
...
| Field | Value |
|---|---|
| `scene_id` | `save-for-italy` |
| `display_name` | 存錢去義大利 |
| `narrative_beat` | 為了義大利，兩個人一起努力、一起等待 |
| `manifest_order` | 6 |
| `phase` | Mid-game |
...
| Event | SFX | Notes |
|---|---|---|
| Scene enter | ambient loop | *(TBD — 期待感、存錢罐聲？)* |
...
| Field | Value |
|---|---|
| `ambient.path` | `res://assets/ambient/save-for-italy.png` |
...
- Cards that persist into the next: TODO (機票？護照？)
...
- `KNOWN_SCENE_IDS` 需新增 `"save-for-italy"`
- [ ] 存錢的方式：記帳、省吃儉用、加班？有什麼特定的小犧牲？
- [ ] 期待感的視覺化：地圖、機票草圖、存錢罐？
- [ ] bar 設計：`savings` 不衰退，靠累積達到目標，強調「努力有成果」的正向感
- [ ] 與 italy scene 的 carry-forward：帶著什麼牌一起去義大利？
...
| `assets/data/scenes/save-for-italy.json` | create |
